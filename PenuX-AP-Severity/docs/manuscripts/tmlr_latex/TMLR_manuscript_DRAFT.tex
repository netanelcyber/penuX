...
\usepackage{tmlr}
% Anonymous submission (default mode): the tmlr package itself replaces the
% author block below with "Anonymous authors / Paper under double-blind
% review" in the compiled output, regardless of the \author{} content --
% do NOT switch to [accepted] or [preprint] until a decision is reached.
...
% Defensive metadata scrub: blank out PDF Author/Title metadata so no
% identity leaks through document properties during blind review either.
...
This work is an exploratory secondary analysis of two public, de-identified datasets and is not validated for clinical use. The main risk of harm is misuse: if a model or finding from this benchmarking study were applied to real patient-care decisions without external validation, prospective evaluation, regulatory clearance, and institutional review, it could contribute to incorrect triage or treatment decisions. We explicitly do not recommend any clinical use of this work. No new patient data were collected, and no re-identification of any individual was attempted or is possible from the data used.

\subsubsection*{Reproducibility Statement}

All classifier configurations used a fixed random seed; the 5-fold cross-validation splits were identical across all 1,982 configurations within each dataset. The two datasets analyzed are third-party datasets publicly released under permissive open-source licenses (MIT and Apache-2.0 respectively). Repository links are withheld from this anonymized version because they identify the corresponding author's account; an anonymized code/results mirror will be provided to reviewers on request during the review period, and full repository links will be restored in the de-anonymized camera-ready version.

% Per TMLR convention, Acknowledgments / Funding / Author Contributions
% are added only after acceptance and de-anonymization -- omitted here.
