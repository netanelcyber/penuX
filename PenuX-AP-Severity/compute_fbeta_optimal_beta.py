"""Find the genuinely optimal F-beta value (not just beta=1 or beta=2) using
the same overfitting-resistant nested + repeated CV procedure as
compute_fbeta_nested.py — sweeps beta in [1.0, 1.25, 1.5, 1.75, 2.0] for the
4 models that led at some threshold in earlier sweeps."""
...
SEEDS = [42, 1, 7]  # 3 repeats (vs 5 in compute_fbeta_nested.py) to bound total compute across 5 betas x 4 models
...
with open("PenuX-AP-Severity/models/fbeta_optimal_beta_results.json", "w") as f:
    json.dump({"betas": BETAS, "results": all_results}, f, indent=2)
print("\nSaved to PenuX-AP-Severity/models/fbeta_optimal_beta_results.json")
