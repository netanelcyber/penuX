...
Typical usage:
    ...
    python penux_unified_nn.py \
      --chartevents dataset/mimic/mimic-iii-clinical-database-demo-1.4/CHARTEVENTS.csv \
      --d-items dataset/mimic/mimic-iii-clinical-database-demo-1.4/D_ITEMS.csv \
      --microbiology dataset/mimic/mimic-iii-clinical-database-demo-1.4/MICROBIOLOGYEVENTS.csv \
      --target pathogen_label --model dnn
"""
...
def normalize_pathogen_name(name: object) -> Optional[str]:
    s = normalize_name(name)
    if s in {'', 'nan', 'none', 'null', 'unknown'}:
        return None
    if s in {'no_growth', 'no_growth_to_date', 'negative'}:
        return 'no_growth'
    if 'escherichia_coli' in s or s == 'e_coli':
        return 'e_coli'
    if 'klebsiella' in s:
        return 'klebsiella'
    if 'pseudomonas' in s:
    ...
    if 'staphylococcus_aureus' in s or 'staph_aureus' in s or s == 's_aureus':
        return 'staph_aureus'
    if 'staphylococcus' in s or 'staph' in s:
        return 'staph_other'
    if 'streptococcus' in s or 'strep' in s:
    ...
    if 'serratia' in s:
        return 'serratia'
    if 'candida' in s or 'yeast' in s:
        return 'candida'
    if 'virus' in s or 'influenza' in s or 'covid' in s or 'coronavirus' in s or 'rsv' in s or 'adenovirus' in s:
        return 'viral'
    return s[:80]


def load_microbiology_table(microbiology_path: str, max_rows: Optional[int] = None) -> pd.DataFrame:
    mb = read_csv_selected(
        microbiology_path,
        wanted_cols=[
            'subject_id',
            'hadm_id',
    ...
    org_col = find_col(mb, ['ORG_NAME', 'ORGANISM', 'LABEL'])
    if org_col is None:
        raise ValueError(f'MICROBIOLOGYEVENTS is missing ORG_NAME. Columns={list(mb.columns)}')

    if find_col(mb, ['HADM_ID']) is None and find_col(mb, ['SUBJECT_ID']) is None:
        raise ValueError(f'MICROBIOLOGYEVENTS is missing SUBJECT_ID/HADM_ID. Columns={list(mb.columns)}')

    time_col = find_col(mb, ['CHARTTIME', 'CHARTDATE'])
    if time_col is not None:
        mb['microbio_time'] = pd.to_datetime(mb[time_col], errors='coerce')
    else:
        mb['microbio_time'] = pd.NaT

    mb['pathogen_label'] = mb[org_col].map(normalize_pathogen_name)
    mb = mb[mb['pathogen_label'].notna()].copy()
    return mb


def add_microbiology_targets(
    timeline: pd.DataFrame,
    encounter: pd.DataFrame,
    microbiology_path: str,
    max_rows: Optional[int] = None,
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    mb = load_microbiology_table(microbiology_path, max_rows=max_rows)
    if mb.empty:
        return timeline, encounter

    join_cols = [c for c in ['HADM_ID', 'SUBJECT_ID'] if c in encounter.columns and c in mb.columns]
    if not join_cols:
        join_cols = [c for c in ['HADM_ID', 'SUBJECT_ID'] if c in timeline.columns and c in mb.columns]
    if not join_cols:
        raise ValueError(
            'Could not join MICROBIOLOGYEVENTS to MIMIC features because no shared SUBJECT_ID/HADM_ID was found.'
        )

    agg = (
        mb.groupby(join_cols + ['pathogen_label'], dropna=False)
        .agg(microbio_count=('pathogen_label', 'size'), latest_microbio_time=('microbio_time', 'max'))
        .reset_index()
    )
    sort_cols = join_cols + ['microbio_count', 'latest_microbio_time']
    ascending = [True] * len(join_cols) + [False, False]
    best = agg.sort_values(sort_cols, ascending=ascending).groupby(join_cols, as_index=False).first()

    keep = join_cols + ['pathogen_label', 'microbio_count', 'latest_microbio_time']
    timeline = timeline.merge(best[keep], on=join_cols, how='left')
    encounter = encounter.merge(best[keep], on=join_cols, how='left')
    return timeline, encounter
...
def filter_valid_targets(df: pd.DataFrame, target_col: str) -> pd.DataFrame:
    target = df[target_col]
    target_str = target.astype(str).str.strip().str.lower()
    invalid = target.isna() | target_str.isin({'', 'nan', 'none', 'null', 'unknown'})
    out = df.loc[~invalid].copy()
    if out.empty:
        raise ValueError(f"No rows remain after filtering missing target values in column '{target_col}'.")
    return out
...
def prepare_tabular_data(
    ...
    df = filter_valid_targets(df, target_col).reset_index(drop=True)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--microbiology", default=None, help="Optional MIMIC-III MICROBIOLOGYEVENTS.csv path for pathogen labels")
    parser.add_argument("--mimic-max-rows", type=int, default=None, help="Optional row cap when reading MIMIC CSVs")

    parser.add_argument("--target", default=None, help="Target column. For raw MIMIC, common choices: aki_stage_label, severe_aki_label, or pathogen_label when --microbiology is provided")
    ...
    parser.add_argument("--test-size", type=float, default=0.15, help="Test split proportion (default 0.15)")
    parser.add_argument("--val-size", type=float, default=0.15, help="Validation split proportion from the full dataset (default 0.15; training becomes 0.70)")
...
def load_input_data(args: argparse.Namespace) -> Tuple[pd.DataFrame, Optional[pd.DataFrame], Dict[str, object]]:
    ...
    if args.chartevents and args.d_items:
        ...
        if args.microbiology:
            timeline_df, encounter_df = add_microbiology_targets(
                timeline_df,
                encounter_df,
                args.microbiology,
                max_rows=args.mimic_max_rows,
            )
        if args.model == "dnn":
        ...
        return working_df, aux_df, {
            ...
            "microbiology": os.path.abspath(args.microbiology) if args.microbiology else None,
            "mimic_max_rows": args.mimic_max_rows,
        }

    raise ValueError("Provide either --csv OR both --chartevents and --d-items. Optionally add --microbiology for pathogen labels.")
...
def main() -> None:
    ...
    if target_col is None:
        raise ValueError(
            ...
            "For raw MIMIC, use --target aki_stage_label or --target severe_aki_label, or use --target pathogen_label with --microbiology."
        )
    ...
    if args.model == "dnn":
        pack = prepare_tabular_data(df, target_col, patient_id_col, time_col, test_size=args.test_size, val_size=args.val_size, seed=args.seed)
        ...
        save_json(
            os.path.join(args.output_dir, f"{prefix}_config.json"),
            {
                ...
                "hidden_dims": hidden_dims,
                "device": str(device),
                "train_size": 1.0 - args.test_size - args.val_size,
